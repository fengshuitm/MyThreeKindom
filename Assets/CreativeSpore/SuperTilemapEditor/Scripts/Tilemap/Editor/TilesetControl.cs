...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilesetControl
    {
        ...
        private class SharedTilesetData
        {
            ...
            private Vector2 _brushesScrollPos;
        ...
        private static Dictionary<Tileset, SharedTilesetData> s_dicTilesetSharedData = new Dictionary<Tileset, SharedTilesetData>();
        private static SharedTilesetData GetSharedTilesetData(Tileset tileset)
        {
            SharedTilesetData sharedData = null;
            if (tileset)
            {
                if (!s_dicTilesetSharedData.TryGetValue(tileset, out sharedData))
                {
                    sharedData = new SharedTilesetData();
                    s_dicTilesetSharedData[tileset] = sharedData;
                }
            }
            return sharedData;
        }

        private bool m_skipFirstRepaint = true;
        ...
        public void Display()
        {
            ...
            //FIX: the tilesScrollPos.y is set to 0 when a tilemap gameObject in selected during the first Pain event, but not if it is selected from a tilemap group
            if (e.type == EventType.Repaint && m_skipFirstRepaint)
            {
                m_skipFirstRepaint = false;
                return;
            }
            ...
            m_sharedData.tilesScrollPos = EditorGUILayout.BeginScrollView(m_sharedData.tilesScrollPos, Styles.Instance.scrollStyle/*, GUILayout.MinHeight(minTileScrollHeight)*/);
            {
                // Scroll Moving Drag
            ...
            else
            {
                ...
                m_sharedData.brushesScrollPos = EditorGUILayout.BeginScrollView(m_sharedData.brushesScrollPos, Styles.Instance.scrollStyle);
                {
